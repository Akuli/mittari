"""Check that links in README.md point to reasonable places.

Mostly copied from https://github.com/Akuli/porcupine/blob/91cf877d5bbda1f6ae93ecfd0a1f8f02bbd13dd2/scripts/check-markdown-links.py
"""
...
def check_https_url(url):
    ...
    if url.startswith(("https://www.biltema.fi/", "https://diy-fever.com/")):
        # Apparently these sites don't like the default user-agent or something
        expected_status = 403
    elif url.startswith(("https://electronics.stackexchange.com/q/", "https://stackoverflow.com/a/")):
        # it's supposed to redirect
        expected_status = 302
    else:
        expected_status = 200
...
def check_link(markdown_file_path, link_target, offline_mode=False):
    ...
    if PROJECT_ROOT not in path.resolve().parents:
        return "link points outside of the project folder"
